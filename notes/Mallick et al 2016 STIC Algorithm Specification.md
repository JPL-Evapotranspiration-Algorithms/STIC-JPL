...
- $g_S$: canopy or aggregated surface conductance (m s$^{-1}$)
...
Notation harmonization note: the 2016 article often uses $g_A$ and $g_C$. In this document, $g_B \equiv g_A$ and $g_S \equiv g_C$ to match the 2014 and 2015 specifications.
...
where $\lambda E$ is latent heat flux and $g_B$, $g_S$ are solved analytically from coupled state equations rather than prescribed by empirical parameterizations.
...
1. $M$ regulates the source-air vapor gradient and therefore $g_S$.
...
2. Compute closure variables ($g_B$, $g_S$, $T_0$ and related vapor terms).
...
1. $g_S$ response to moisture status and atmospheric demand.
2. $g_B$ modulation of evaporation transfer.
3. Conductance-ratio behavior ($g_S/g_B$) as an indicator of coupling regime.
4. Seasonal and drought hysteresis in the $\lambda E_T$-$g_S$ relationship.
...
2. Dry-season and drought periods show stronger biophysical control, with increased moisture limitation through $g_S$.
3. Similar canopy-atmosphere coupling can emerge across different plant functional types when soil-moisture stress reshapes $g_S/g_B$.
