...
| $ho$ | Density of air ($	ext{kg}\ 	ext{m}^{-3}$) |
...
$$\lambda E = rac{ho c_P}{\gamma}g_B(e_S - e_A) = M E_P = rac{ho c_P}{\gamma} M g_B(e_S^* - e_A) \qquad (3)$$

From Eq. (3), a physical expression for $M$ is given in terms of the vapor pressure gradients:
$$M = rac{(e_S - e_A)}{(e_S^* - e_A)} \qquad (4)$$
...
By analogy to the dew point temperature, $T_D$, if the surface air is brought to saturation without affecting $e_S$ then $e_S = f\{T_{SD}\}$. Thus, $T_{SD} < T_S$ for unsaturated surface and $T_{SD} ightarrow T_S$ as the surface tends to saturation. Therefore, $e_S$ is the surface saturation vapor pressure at some notional surface dew point $T_{SD}$. Although $T_{SD}$ cannot be directly measured, it can be derived from the slope of the relevant temperature-saturation vapor pressure curve. Therefore, from Eq. (4), Fig. 2, and following Monteith (1965), $M$ can be written as follows,
$$M = rac{(e_S - e_A)}{(e_S^* - e_A)} = rac{s_1(T_{SD} - T_D)}{s_2(T_S - T_D)} \qquad (5)$$

where $s_1$ and $s_2$ are the slopes of the saturation vapor pressure and temperature between $(T_{SD} - T_D)$ versus $(e_S - e_A)$ and $(T_S - T_D)$ versus $(e_S^* - e_A)$ relationship. For a dry surface $T_S \gg T_{SD}$, $e_S^* \gg e_S$, $(T_S - T_D) \gg (T_{SD} - T_D)$ and $M ightarrow 0$. For any wet surface, $e_S^* \cong e_S$, $T_S \cong T_{SD}$, $(T_S - T_D) \cong (T_{SD} - T_D)$ and $M ightarrow 1$. According to Eq. (5), when condensation occurs, the surface temperature goes below the ambient air temperature and $e_S < e_A$, otherwise $e_S \ge e_A$ and $e_S \le e_S^*$. Hence, $T_{SD}$ falls somewhere between $T_S$ and $T_D$ (Fig. 2a) and can be computed by linearizing the saturation vapor pressure curve between the two levels $(T_D, T_{SD})$ and $(T_{SD}, T_S)$ (Monteith, 1965).
...
$$s_1 = rac{(e_S - e_A)}{(T_{SD} - T_D)} \qquad (6)$$
$$s_3 = rac{(e_S^* - e_S)}{(T_S - T_{SD})} \qquad (7)$$

Combining Eqs. (6) and (7) we can find an expression of $T_{SD}$:
$$T_{SD} = rac{(e_S^* - e_A) - s_3 T_S + s_1 T_D}{(s_1 - s_3)} \qquad (8)$$
...
$$\lambda E_P^* = rac{s\Phi + ho c_P g_B D_A}{s + \gamma} \qquad (11)$$
...
$$\lambda E_{PT}^* = rac{lpha s \Phi}{s + \gamma} \qquad (12)$$

where $lpha$ is a compensation factor introduced due to neglecting the adiabatic term in the Penman equation. They decided that for a large saturated landscape with minimum advection the best estimate of $lpha$ is 1.26 (Priestley & Taylor, 1972). Therefore, the interdependence of $\lambda E_{PT}^*$ and $\lambda E_P^*$ is not by chance. When the underlying surface is wet, the air is close to saturation and evaporation is dominated by the radiation term of the Penman equation. But, $\lambda E_{PT}^*$ never equals $\lambda E_P^*$ because of the involvement of the 'drying power' term in $\lambda E_P^*$.

It should also be emphasized that although the Priestley-Taylor $lpha$ is an input into our scheme, the sensitivity analysis over a large range of $lpha$ values revealed that STIC is only mildly sensitive to $lpha$ (see Section 4.1 and Fig. 5b).
...
$$\Lambda = rac{2lpha s}{2s + \gamma \left(2 + rac{g_B}{g_S}ight)} \qquad (13)$$
...
$$g_B = rac{\Phi}{ho c_P \left(\Delta T + rac{e_S - e_A}{\gamma}ight)} \qquad (14)$$
$$g_S = g_B rac{(e_S - e_A)}{(e_S^* - e_S)} \qquad (15)$$
$$\Delta T = \left(rac{e_S - e_A}{\gamma}ight)\left(rac{1 - \Lambda}{\Lambda}ight) \qquad (16)$$
$$\Lambda = rac{2lpha s}{2s + \gamma \left(2 + rac{g_B}{g_S}ight)} \qquad (17)$$
...
$$T_D = rac{c \cdot \gamma_M(T_A, R_H)}{b - \gamma_M(T_A, R_H)} \qquad (19)$$
$$\gamma_M(T_A, R_H) = \log\left(R_H \cdot \exp\left(\left(b - rac{T_A}{d}ight)\left(rac{T_A}{c + T_A}ight)ight)ight) \qquad (20)$$
...
Since the Priestley-Taylor parameter ($lpha$) was involved in the entire scheme and there are reports of wide variability of $lpha$ (Komatsu, 2005), the sensitivity of STIC to $lpha$ was also tested for large variations in $lpha$. The reported values of $lpha$ in the literature are found to vary between 1 and 1.5 (Baldocchi & Xu, 2007). Taking this range we found that for $\lambda E$ the magnitude of $S_V$ varied from 10 to 13% and for $H$ it varied from 9 to 16% (Fig. 5b). This suggests that STIC is not particularly sensitive to $lpha$ and assigning $lpha = 1.25$ does not introduce significant error in the $\lambda E$ and $H$ outputs. Maximum RMSE difference in $\lambda E$ and $H$ was 10 and 15 $W\ m^{-2}$, respectively.

Sensitivity of STIC was also tested for the atmospheric vapor pressure ($e_A$) and both the fluxes were not very sensitive to $\pm10\%$ uncertainty in the $e_A$ values (Fig. 5c). The magnitude of $S_V$ varied between 5-6% for $\lambda E$ and 3-9% for $H$. The typical errors of water vapor retrieval from the current MODIS atmospheric data and Atmospheric Infrared Sounder (AIRS) are 5-10% (Gao & Kaufman, 2003) and 10% (Tobin, Revercomb, Knuteson, Lesht, Strow, Hannon, et al., 2006), respectively. With the advent of new sensor technology and the scheduled launch of a future geostationary interferometer sounder GEOSTAR, the water vapor retrieval error will be within 5%, which will facilitate the spatial implementation of STIC for large area mapping of surface fluxes.
...
The predicted versus measured $\lambda E$ and $H$ show good overall agreement in all the 30 tower sites (Table 3) with a relative cumulative error $(	ext{observed} - 	ext{estimated})/	ext{observed}$ in $\lambda E$ and $H$ of the order of 4% and 3% respectively. The root mean square error (RMSE) of individual sites varied between 10.70 and 59.67 $W\ m^{-2}$ with measured versus predicted correlation coefficients ($r$) between 0.55 and 0.97 (Table 3). The RMSE and $r$ of $\lambda E$ from all the sites were 37.79 $W\ m^{-2}$ (11% error) and 0.89 (Table 3). The performance of STIC was consistent over all sites except FOG and DEM where the RMSE was relatively higher (see Table 3). Pooled RMSE and $r$ for $H$ for the 30 sites were 37.74 $W\ m^{-2}$ (9% error) and 0.91. RMSE's of individual sites varied between 12.11 and 58.48 $W\ m^{-2}$ with $r$ varying between 0.55 and 0.96 (Table 3).
...
| Site | N | $\lambda E$ RMSE ($W\ m^{-2}$) | $\lambda E$ MB ($W\ m^{-2}$) | $\lambda E$ r | Mean Obs $\lambda E$ | Mean STIC $\lambda E$ | $\lambda E$ Err (%) | $H$ RMSE ($W\ m^{-2}$) | $H$ MB ($W\ m^{-2}$) | $H$ r | Mean Obs $H$ | Mean STIC $H$ | $H$ Err (%) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
Daily $\lambda E$ was computed following the principle of diurnal conservation of evaporative fraction following Anderson et al. (2007). The estimated evaporative fraction $(\Lambda = \lambda E / (\lambda E + H))$ from STIC was multiplied by the daily net available energy $(\Phi = R_N - G)$ which was available at each EC site. Thus daily $\lambda E = \Lambda \Phi$ and daily $H = (1 - \Lambda)\Phi$. Analysis of the daily predicted versus measured $\lambda E$ using MODIS Terra revealed RMSE to be 5.59-34.89 $W\ m^{-2}$ with $r$ varying between 0.61 and 0.98 (Table 4). Overall RMSE and $r$ was 15.70 $W\ m^{-2}$ and 0.89, with percent error ranging from 1 to 31% (Table 4). Pooled RMSE and $r$ for $H$ over all the sites were 14.52 $W\ m^{-2}$ (6% error) and 0.88, with a range varying from 5.64-25.72 $W\ m^{-2}$ and 0.48-0.96, respectively.
...
| Site | N | $\lambda E$ RMSE ($W\ m^{-2}$) | $\lambda E$ MBE ($W\ m^{-2}$) | $\lambda E$ r | Mean Obs $\lambda E$ | Mean STIC $\lambda E$ | $\lambda E$ Err (%) | $H$ RMSE ($W\ m^{-2}$) | $H$ MBE ($W\ m^{-2}$) | $H$ r | Mean Obs $H$ | Mean STIC $H$ | $H$ Err (%) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
where $\Phi =$ net available energy $(\cong R_N - G)$, $H =$ sensible heat flux, $\lambda E =$ latent heat flux, $R_N =$ net radiation, $G =$ conductive surface heat flux or ground heat flux. All the fluxes have units in $W\ m^{-2}$. The sensible and latent heat flux can be expressed in the form of aerodynamic transfer equations (Boegh & Soegaard, 2004; Boegh et al., 2002) as follows:
$$H = ho c_P g_B \Delta T \qquad (A2)$$
$$\lambda E = rac{ho c_P}{\gamma} g_B (e_S - e_A) \qquad (A3)$$
